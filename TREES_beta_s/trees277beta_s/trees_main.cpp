...
int main()
{
	...
	int i, j;
	String pfile, param_file_modules, dfile, cfile, sfile, hfile, efile, vfile; //filenames
	ifstream fin;
	ofstream chnout, effout, fluxout, hydrout;
	Data_Store indata;
	State_Store state;
...
//end user input
		...
		effout << "output file: " << cfile << endl;
        	...
        	cout << endl;
        	cout << ">>> TREES, v. 2.7.7, stochastic parameter mode <<< \n\n";
        	cout << endl;
...
//switch likelihood function each time covariance is updated
	...
	else if (rp == 0)//simulation with constant parameters
	{
		...
		fluxout << "Rgrowth\tleafNSC\tstemNSC\trootNSC\tchloroStarch\tchloroSugar\twaterStress\tlitterH2O\t";
		for (i = 0; i < treesParams.rmodules; i++)
                {
                        fluxout << "theta" << i << "\t";
                }
		...
		fluxout << "J_sun\tJ_shd\tAsun\tAshd\tAv\tAj\tphi2\tbeta\ts\talpha\tLsun\tLshd\tTsun\tTshd\tDsun\tDshd\tCi_sun\t";
...
*/
		...
		hydrout << "\n";
		simulator(indata, state, currp, treesParams, Ecframe, NEEframe, fluxout, hydrout);
		fluxout.close();
		hydrout.close();
