                                            ...
                                            struct farqout& out)
{
    ...
    double phi2;            // PSII operating efficiency
    double phi2_300;        // PSII operating efficiency at 300 umol m-2 s-1 PAR  (currently implemented using thetaJ placeholder in par file
    double betaA;           // decay rate PSII operating efficiency under increasing PAR
    double alpha;           // maximum PSII operating efficiency as PAR approaches zero
    ...
    double phi2_hyd;        // PSII operating efficiency at 300 umol m-2 s-1 PAR under hydrualic constraint
	double aa,bb,cc,det;
...
//therefore, Ile no longer used, but embedded in phiJ
     ...
     */
    // modification to photosynthesis routine to describe estimate eletron rate using paratmers from chl fluoresnce and the realtionship between phi2 and leaf water potential (JRP 2/2018)
    ...
    /// beta decay implimentaion - version 2 beta decrease with leaf psi  and s changes with beta
    phi2_hyd = m_phi*(daily_md_lwp) + phi2_300;
    betaA =(log((phi2_hyd-kappa)/(alpha-kappa)))/300;  /// calc decay rate from exp decay fuction based on phi2_hyd
    s= (0.31 / (1 + 0.0015*exp(-3000*betaA)));   /// logit fuction to reduce s with beta logit<-a / (1 + b*exp(-k*bb))
