...
#define MAXITS 30   //maximum number of iterations allowed in solvewater
...
/*
		BiogeochemicalCycles(BiogeochemicalCycles& bg,
...
*/
	...
	//method to compute NSC fluxes
		void computeNSCfluxes(double kratio,
				      double* kratio_vector);
		void computeNSCfluxes(double kratio,
				      double kratioRoot);

	//method to compute N fluxes
		void computeNfluxes(double kratio,
			            double* kratio_vector);
		void computeNfluxes(double kratio,
			            double kratioRoot);
	...
	//method to update stem carbon pools
		void updateStemCarbonNitrogenPools(double N_avail_rate_plant,
						   double kratio,
					   	   double nscRatio,
                                           	   double rgrowth,
...
public:
   	...
   	void setup( outputStruct &ecrit_k_psi,
                    outputStruct &k_p_e,
                    double ecritOut[],
                    double pc[],
                    double klpred[],  
...
totmodules,
                     int latot, int rmodules, double rr, double rd, double Fabs,
...
struct sim_out
    ...
    double md_lwp; // midday lwp
    double daily_pd_lwp; // predawn lwp
    double daily_md_lwp; // midday lwp
	double sum_vpd;
	double GSI[21];
	double GSImean;
	double leaf_growth;
...
struct sim_out simulation_functions(
				...
				double cpp[][ULAT], double ku[][ULAT],
                    		double kl[][ULAT], double f[][ULAT], double cc[][2], 
				double psinode[][NMAX], double psim_outin[][NMAX],
                    		double jmatrix[][NMAX], double jmatrix2[][NMAX], 
				double percent[], double rssoil[], double rs[],
                    		double dp[], double ff[], int col[], int row[], 
				int indxx[], double subtract[], double pressure[],
