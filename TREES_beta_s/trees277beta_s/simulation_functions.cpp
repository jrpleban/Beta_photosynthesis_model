...
//Get root depths from the plant module parameterization (see below)
	...
	int rmodules;
        ...
        double field_capacity = treesParams.field_capacity;
...
//display current status of the larget live LAI and stress on the the canopy
	ti.get_time(year, yday, hour, min);
        if ((hour+min) == 0.0 && treesParams.useHydraulics == true && !silent)
	{
		cout << endl << "-------- ";
		cout << year << ":" << yday;
		cout << " --------" << endl;
...
//This routine probably works best if the simulations begins before leaf flush
...
//added to handled predawn leaf water potential
		if ((hour+min) == 3.0 && treesParams.useHydraulics == true)
		{
			simOut.pd_lwp = simOut.Leaf_Psi;
		}
    

//added to handled midday leaf water potential
        if ((hour+min) == 13.0 && treesParams.useHydraulics == true)
        {
            simOut.md_lwp = simOut.Leaf_Psi;
        }
        
        
        //MOVING WINDOW to trait leaf water potential status JRP
		...
		if (hour+(double)min/100.0 == 23.5 && ts >= 48)
		{
			...
			simOut.daily_pd_lwp = simOut.pd_lwp; // predawn LWP
            simOut.daily_md_lwp = simOut.md_lwp; // middawn LWP
			//simOut.pd_lwp = 0.0;
            //simOut.md_lwp = 0.0;
			...
			else
			{
             ...
             cout << "daily_pd_lwp = " << daily_pd_lwp;
            cout << "daily_md_lwp = " << daily_md_lwp;
...
//NOTE: NSC for leaf expansion is now implicit in lai_pot from growth allocation
//      Growth respiration component is deduced from NSC here  - DSM June 2014
//         amounts to 14% of NSC going to respiration

//keep a running tally of nitrogen demand - DSM 11/15
...
//********************************************************************************************
	...
	canopy_store = state.get_val_at(CANOPY);  //how much water is there on the canopy?
...
//
	...
	infiltration(eff_precip, state, rootDepth, thetaSoil, bypassFlow, rmodules,
			litter_capacity, porosity, pore_size_index, field_capacity, 
...
//force high boundary layer conductance - DSM August 2011
...
//Compute water stress by root layer for use when running non-hydraulic mode
				waterStress[i] = theta_gsref_reduction(porosity, bubbling_pressure, pore_size_index, 
									residual, LWP_spring_minimum, LWP_stomatal_closure, 
									thetaSoil[i], field_capacity);
...
//calculate canopy evaporation
	...
	if (precip < 0.0005)
	{
		...
		{
			can_evap_sun = do_pm(Rnet_sun, simOut.T_sun, D*(1.0-canopy_wetness), p_air, gHr, gva);
			can_evap_shd = do_pm(Rnet_shd, simOut.T_shd, D*(1.0-canopy_wetness), p_air, gHr, gva);
...
//give a report at 1 pm each day
			ti.get_time(year, yday, hour, min);
/*
			if ((hour+min) == 13.0 && treesParams.useHydraulics == true && !silent)
			{
                		cout << "Gs_sun0 = " << gvc_sun/lai_sun;
			}
*/
            	}
//CALL SPERRY UNTIL CONVERGENCE IS REACHED OR HYDRAULIC FAILURE IS DETECTED - SUBSEQUENT ITERATIONS ONLY
            	else
            	{
...
//Call the soil-plant hydraulic model (Sperry et al 1998 PCE)
			        	...
			        	Ysoil, EcStateInner[is], Kl, leafpsi, Ecrit,
                                	HydraulicModelFailCond, PsiCrit,
                                	hydraulicModel_flag, treesParams, ecrit_k_psi, k_p_e, ecritOut, 
					...
					kshoot, dslat, dsax, drlat, drax, l, ksat, bsat,
                                        ccsat, newb, newc, 
					ip, b, b1, n, n1, r, vu, vl, cf, pe, ws_, ks2,
                                        p, dpp, jl, wnu, wu, wnl, wl, cpu, cpl, cpp, ku, kl, 
					f, cc, psinode,
                                        psimin, jmatrix, jmatrix2, percent, rssoil, 
					rs, dp, ff, col, row,
                                        indxx, subtract, pressure, plc, plcweib, 
					rsquare, soilpsi, al, ar,
                                        saturatedKs, ptarg, 
					e, rr, rzw, rd, einc, SoilRhizElements, dt, gmd,
                                        gsd, bkd, fs, fc, Fabs, 
					cpplant, saxlat, raxlat, rfract, kla, aral, latot, ratot, 
					shootElements, ktotal, soilpsimin, ShootModules, rootElements,
                                        RootModules, totmodules, 
...
//This if{} is suspect. Negative KL can occur at night when net flow 
			...
			{
/*
				if (Kl < -treesParams.saturated_kl_for_whole_plant)
				{
...
//Temporary - Have a peek at 1pm
/*
			if ((hour+min) == 13.0 && treesParams.useHydraulics == true && !silent)
			{
				if (Soil_Psi > 0.5*(treesParams.pd_at_sat_kl+Soil_Psi))
				{
					cout << "Gs_sun = (" << Kl << "/" << D0_sun/p_air*1000.0 << 
						")*(" << 0.5*(treesParams.pd_at_sat_kl+Soil_Psi) << 
						" - " << leafpsi << ") = " << gvc_sun/lai_sun;
				}
				else
				{
					cout << "Gs_sun = (" << Kl << "/" << D0_sun/p_air*1000.0 << 
...
//(DSM May 2010)
        	Rnet_sun = Rabs_sun - G;
		if (lai_sun >= 0.5) //Sunlit leaf energy balance has numerical problems at high solar zenith angle (horizon)
		{
...
//In case you want to include leaf boundary-layer conductance, here you go:
...
//calculate sun leaf photosynthesis, intercellular CO2, and stomatal conductance
		in.t = simOut.T_sun;
        //md_lwp =simOut.md_lwp;

		A_sun = bgc.photosynthesis(treesParams, Jmax_mult, thetaJ, adj_phiJ_sun, simOut.daily_md_lwp,in, out);
		A_sun = (1.0-canopy_wetness)*A_sun;	
		gvc_sun = out.g * 1.6 * 42.0; //Molar conversion for CO2 to H2O and m/s to mol/m2/s

		if (gvc_sun < 0.0001*Gsref0)
		{
			gvc_sun = 0.0001*Gsref0;
		}
...
//adjust for the proportion of canopy that is still alive
//		gvc_sun *= treesParams.live_lai / treesParams.lai_at_sat_kl;
        ...
        simOut.Av = out.Av;
        simOut.Aj = out.Aj;
        simOut.betaA = out.betaA;
        simOut.phi2 = out.phi2;
        simOut.s = out.s;
        simOut.alpha = out.alpha;
...
//calculate shade leaf photosynthesis, intercellular CO2, and stomatal conductance
		in.t = simOut.T_shd;
		A_shd = bgc.photosynthesis(treesParams, Jmax_mult, thetaJ, adj_phiJ_shd, simOut.daily_md_lwp,in, out);
		A_shd = (1.0-canopy_wetness)*A_shd;	
//Molar conversion for CO2 to H2O and m/s to mol/m2/s
		gvc_shd = out.g * 1.6 * 42.0; 
...
//adjust for the proportion of canopy that is still alive
//		gvc_shd *= treesParams.live_lai / treesParams.lai_at_sat_kl;
...
//call P-M
		Ec_sun = (1.0-0.5*canopy_wetness)*
				do_pm(Rnet_sun, simOut.T_sun, 0.5*(D0+D0_sun), p_air, gHr, gv_sun);
		Ec_shd = (1.0-0.5*canopy_wetness)*
		...
		if (ModelStatus == 0)
		{
			...
			Ec_max = EcConversion_TtoS(Ec_max,treesParams.Al);
		}
...
//Note: Ec_ goes into HydraulicModel routine where it is multiplied by treesParams.Al 
//    contributed by each element to get flow rate
//   [ Must use treesParams.Al ]
		Ec_ = EcConversion_TtoS(pm_Ec, treesParams.Al);
		is++;
...
//THIS IS THE CRITERIA REQUIRED FOR THE MODEL TO CONTINUE CALLING SPERRY******//
        } while ((Delta > Ec_max*0.01) && (Converged == false));

//update E increment based on current Ecrit
...
//model did not converge
        ...
        {
            simOut.Ecrit = Ecrit;
            //simOut.Ec = Ec_*treesParams.Al/lai;
            simOut.Ec = Ec_;
            simOut.Psi_Crit = PsiCrit;
            simOut.WPlant_K = Kl;
            simOut.Leaf_Psi = leafpsi;
...
//success
	...
	Ec_t = canopy_cover*EcConversion_StoT(Ec_, treesParams.Al);
	if (Ec_t < 0.0)
	{
		Ec_t = 0.0;
	}
...
//
	for (i = 0; i < rmodules; i++)
	{
		if (treesParams.useHydraulics == true)
		{
			simOut.rhizFlux[i] = k_p_e.rhizFlux[i+2]*ar[i+3]/lai;
		}
		else
		{
...
//--> END OF SPERRY-TREES COUPLED PARTS
...
//Calculate soil water flow conductance to limit the rate of evaporation from the soil surface
		...
		if (is_bryophyte == 0) //This version is for mineral and/or organic soils
		{
				...
				field_capacity, ks, pore_size_index, capFringe, porosity, mineral_fraction);
...
//  flux is scaled with deep root zone soil moisture - hmmm, crude
			gv_soil = soil_cond_rate(dZ, Dsoil, t_nearSurface, residual, 
					thetaSoil[(int)rmodules-1], field_capacity, ks, 
...
//assume albedo of 0.9 for litter
        	Rnet_soil = 0.9 * (Qtotal - Rabs_sun - Rabs_shd) - Rth;
		litter_store = state.get_val_at(LITTER);
		evaporative_fraction = litter_store/litter_capacity;
...
//********************************************************************************************
	...
	if (thetaSoil[0] > porosity || (Zw-capFringe-cumDroot[0]) < 1.0E-6)
	{
		thetaSoil[0] = (cumDroot[0]-(Zw - capFringe))/rootDepth[0]*porosity +
				((Zw - capFringe) - (cumDroot[0] - rootDepth[0]))/rootDepth[0] * field_capacity;
	}
	for (i = 1; i < rmodules-1; i++)
	{
		...
		if (thetaSoil[i] > porosity || (Zw-capFringe-cumDroot[i]) < 1.0E-6)
		{
			thetaSoil[i] = (cumDroot[i]-(Zw - capFringe))/rootDepth[i]*porosity +
				((Zw - capFringe) - (cumDroot[i] - rootDepth[i]))/rootDepth[i] * field_capacity;
	...
	if (thetaSoil[rmodules-1] > porosity || (Zw-capFringe-cumDroot[rmodules-1]) < 1.0E-6)
	{
		thetaSoil[rmodules-1] = (cumDroot[rmodules-1]-(Zw - capFringe))/rootDepth[rmodules-1]*porosity +
				((Zw - capFringe) - (cumDroot[rmodules-1] - rootDepth[rmodules-1]))/rootDepth[rmodules-1] * field_capacity;
		//thetaSoil[rmodules-1] = porosity;
	}
...
//convert from umol m-2 leaf area to kgC ha-1 30min-1 (explained below)
...
//  Modified to handle 10 size classes of roots in each soil-root layer - DSM July 2015
	...
	for (i = 0; i < rmodules; i++)
	{
		if (treesParams.useHydraulics == true)
		{
			kratio = k_p_e.latK[2+i]/ksat[i+3][1];
		}
		else
		{
			kratio = water_stress;
...
//reduce respiration in low oxygen
		...
		for (int k = 0; k < 10; k++)
		{
			...
			if (0.99*bgc.getRootNSC(i, k) > R_root_segment)
			{
...
//activate growth when there is chloroplast NSC
	...
	//growth_resp_frac = growth_resp_proportion*kratio;
	rgrowth = (bgc.getChloroplastSugar()+bgc.getChloroplastStarch())/48.0 * growth_resp_frac;
...
//this growth will:
//1. determine the target LAI for the next year, but resources won't be consumed until actual leaf expansion
...
//  or decrease it if NSC is in short supply
	...
	if (treesParams.usePhenology == true)
	{
		nsc_base = (lai_pot-state.get_val_at(CURRENTTARGETLAI))/treesParams.SLA*10000.0 + 
			0.5*(treesParams.leafNSCscalar*lai_pot/treesParams.SLA*10000.0 +
			treesParams.leafNSCscalar*totalRootCarbon + 
			0.4*treesParams.leafNSCscalar*bgc.getLiveStemCarbon());
	}
	else
	{
		nsc_base =  0.10*(treesParams.leafNSCscalar*bgc.getLeafBiomassCarbon() +
...
//
	...
	if (t_root >= -50.0)
	{
		bgc.computeLeafAllocation(treesParams, newc, N_avail_rate_plant, kratio, nsc, psn, 
				nscRatio, rgrowth, leafCfraction, lai, stressedLeafLifeSpan, SLA);
...
/*
		if (yday == 1 && hour == 0 && min == 0)
		{
			state.set_val_at(lai_pot, FORECASTTARGETLAI);
		}
		if (lai_pot > state.get_val_at(FORECASTTARGETLAI))
		{
			state.set_val_at(lai_pot, FORECASTTARGETLAI);
		}
*/
...
// Warning: Don't change treesParams.Al, as this one is used throughout HydraulicModel model

//What to do about root-leaf area ratio
//   -> Suggest having it approach double as leaf area approaches zero to prevent numerical issues
//   -> Update root carbon content as well
		treesParams.aral_at_sat_kl = treesParams.Ar_Al;

//Update stem C and N and add to the N demand
		bgc.updateStemCarbonNitrogenPools(N_avail_rate_plant, kratio, nscRatio, rgrowth, 
...
//Move NSC and N as a function of concentration gradients and hydraulics
	bgc.computeNSCfluxes(kratio, kratio_vector);
	bgc.computeNfluxes(kratio, kratio_vector);
	//bgc.computeNSCfluxes(kratio, kratioRoot);
	//bgc.computeNfluxes(kratio, kratioRoot);
...
//	R_total = (R_bg + R_stem) / 0.012 * pow(10.0,6.0) / pow(10.0,4.0) / 1800.0;
	R_total = (simOut.leaf_growth_respiration + 0.14*rgrowth + R_bg + R_leaf + R_stem + 
...
//compute nitrogen uptake 
	...
	bgc.computeRootNitrogenUptake(UP_neg, UP_pos, treesParams, thetaSoil, Rflux, ratot, 
					field_capacity, N_neg_demand, N_pos_demand);
