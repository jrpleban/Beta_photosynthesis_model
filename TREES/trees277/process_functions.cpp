...
{
        return 101.3*exp(-(altitude/8200.0));
}
...
{
        return mol_density(pressure, tcelsius)*mm_air*pow(10.0,-3.0);
}

double lheat_vap(double tcelsius) //Jkg-1
{
        return (2.501 - (2.361*pow(10.0,-3.0)*tcelsius))*pow(10.0,6.0);
}
...
//eq 11.1 - C&N
        ...
        return acos(ztemp); //the zenith angle
}
...
{
        return ((1.0 - 0.84*fcloud)*(0.0000092*tK*tK) + (0.84 * fcloud));
}
                    ...
                    double zenith)
{
        ...
        double R, K, Qo, atm_trns, fdiffuse; //variables used here
...
{
        double numerator, denominator, omega_zen, extinction;
        numerator = sqrt((l_angle*l_angle)+tan(zenith)*tan(zenith));
        denominator = l_angle + 1.774*pow((l_angle+1.182),-0.733);
	omega_zen = omega/(omega+(1.0-omega)*exp(-2.2*pow(zenith,p_crown))); //Eq. 15.35 C&N 1998
	extinction = (numerator/denominator)*omega_zen;
        return extinction;
}
...
/*
	...
	Kd = cnpy_diff_ext(l_angle, lai_below, omega, p_crown);
*/
...
//absorbed flux density
        ...
        //Qsun = alpha_PAR*(Kb*tQob_PAR+tQd_PAR+tQsc_PAR) + alpha_NIR*(Kb*tQob_NIR+tQd_NIR+tQsc_NIR);
        Qsun = alpha_PAR*(Kb*dQb_PAR+tQd_PAR+tQsc_PAR) + alpha_NIR*(Kb*dQb_NIR+tQd_NIR+tQsc_NIR);
 
//incident flux (this is the mod 042004)
        //PAR_sun = Kb*Qob_PAR+Qd_PAR+Qsc_PAR; //average
        //PAR_sun = Kb*Qob_PAR+tQd_PAR+tQsc_PAR; //average
...
//lai partition
/*
        lai_sun = (1.0-exp(-Kb*lai_below))/Kb;
...
{
        double gr = 4.0*es*sigma*pow(C2K(ta), 3.0)/cp_air; //corrected unit to molm-2s-1
        return gr;
}
...
//Determine water potential equivalent of field capacity
	...
	//PSI_crit = bubbling_pressure/10000.0;
...
cout << cumDepth-(Zw-capFringe) << '\t' << Zw-capFringe-(cumDepth-rootDepth[i]) << endl;
                                 ...
                                 double theta)
{
	...
	return(Sr/42.0); //convert to molar units
}
...
//        psi2 = air_entry_pressure;
//calculate water potential at air-soil surface, assuming atmospheric VPD just above soil surface
//	psi2 = -VPD * 1000.0; //convert to water potential in m H2O
	//psi2 = -air_entry_pressure*pow(pow(0.00000001,-1.0/m)-1.0,1.0/n);
	psi2 = -2243.95;
...
//                                          m/s = m/s/(kPa/kPa) * m/m
	...
	cond = cond*42.0; //convert from m s-1 to mol m-2 s-1
        if (cond < 1.0E-10)
	{
               cond = 1.0E-10;
	}
...
//update soil theta for the ith layer
                        ...
                        else
                        {
                                drain += unsat_zone_drainage(rootDepth[i], rootDepth[i], pore_size_index,
                                                                ks/kinter, updatedTheta, porosity,
                                                                field_capacity, residual, capFringe,
                                                                porosity, mineral_fraction);
                                drain *= drainScalar;
                        }
...
//calculate drainage using Darcy's Law
		...
		//drain = ku*((psi2-psi1+1.0)/(Z2-Z1)+1.0);
